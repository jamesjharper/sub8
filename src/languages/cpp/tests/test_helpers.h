...
inline uint32_t bits_from_float(float v) {
  uint32_t b;
  std::memcpy(&b, &v, sizeof(b));
  return b;
}

inline uint64_t bits_from_float(double v) {
  uint64_t b;
  std::memcpy(&b, &v, sizeof(b));
  return b;
}

inline float float_from_bits(uint32_t u) {
  float f;
...
inline float float16_subnormal_value(uint16_t frac /* 1..1023 */) { return std::ldexp(1.0f, -24) * static_cast<float>(frac); }

#define MAX_BITS(n) ((n) >= 64 ? ~0ULL : ((1ULL << (n)) - 1ULL))

inline void require_strings_match(const std::string_view &a, const std::string_view &b) {
  if (a == b) {
